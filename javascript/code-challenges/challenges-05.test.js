...
const addValues = (arr) => {
  let output = arr.reduce((prev, curr) => { return prev += curr }, 0);
  return output;
};
...
const addPurchases = (arr) => {
  let output = arr.reduce((prev, curr) => { return prev += curr.purchasePrice }, 0);
  return output;
};
...
const reversedString = (str) => {
  let split = str.split('');
  let output = split.reduce((acc, value) => {
    return value + acc;
  })
  return output;
};
